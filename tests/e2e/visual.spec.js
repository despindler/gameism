...
test('authenticated office simulation renders and main workflow works', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Help and game guide' }).click();
  ...
  await expect(drawer.locator('#timeline-summary')).toContainText('active');
  await expect(drawer.locator('#timeline-list')).toContainText('Phishing attempt against EHR access');
  await expect(drawer.locator('#timeline-list')).toContainText('Backup restore failure');
  await expect(drawer.locator('#timeline-list')).not.toContainText('Office function');
  ...
  await expect(page.getByRole('heading', { name: 'Open Findings' })).toHaveCount(0);
  ...
  await drawer.getByRole('button', { name: 'Actions for Phishing attempt against EHR access' }).click();
  await expect(drawer.getByRole('menuitem', { name: 'Open event' })).toBeVisible();
  await drawer.getByRole('menuitem', { name: 'Open asset' }).click();
  const directAssetDialog = page.locator('#context-modal');
  await expect(directAssetDialog).toBeVisible();
  await expect(directAssetDialog.getByRole('heading', { name: 'Cloud EHR' })).toBeVisible();
  await directAssetDialog.getByRole('button', { name: 'Close', exact: true }).click();
  await expect(directAssetDialog).toBeHidden();
  await page.getByRole('button', { name: 'Timeline' }).click();
  await drawer.getByRole('button', { name: 'Actions for Phishing attempt against EHR access' }).click();
  await drawer.getByRole('menuitem', { name: 'Start event' }).click();
  await expect(page.getByText('Timeline event started.')).toBeVisible();
  await drawer.getByRole('button', { name: 'Open Phishing attempt against EHR access details' }).click();
  const eventDialog = page.locator('#context-modal');
